...
we have four row-based test cases: 1) the input \code{\$age=25,
  \$gender=`Female', \$country=`USA'} and the output of \code{Alice}'s
record, 2) that input and the absence of \code{Bob}'s, 3) that input
and the output of \code{Carol}'s, and 4) that input and the output of
\code{Daniel}'s.
